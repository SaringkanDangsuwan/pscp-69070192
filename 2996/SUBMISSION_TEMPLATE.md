...
```text
I receive a string input and then I display the output by reversing the characters. For example, if the input is "Messi", the output will be "issem".
...
```text
I see that the problem requires taking an input value, so I create a variable to store it. Then I use `[::-1]` to reverse the string, use `.lower()` to convert all characters to lowercase, and finally display the result.
```
