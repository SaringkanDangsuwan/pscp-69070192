...
```text
โจทย์ให้ฉันคำนวณระยะการกระโดดของกบ ว่าจะกระโดดกี่ครั้ง และ ว่าจะไปถึงตามเป้าหรือไม่ และระยะกระโดดของกบจะลดลงทุกๆ 2 เมตร

Input:
รับจำนวนเต็ม X Y ในบรรทัดเดียว X คือระยะการกระโดครั้งแรก Y คือระยะทางเป้าหมาย

Output:
จำนวนครั้งที่กบต้องกระโดด หากกระโดดไม่ถึงระยะ Y ให้แสดงผล -1

Constraints:
X อยู่ระหว่าง 1-1000 Y อยู่ระหว่าง 1-100000
```
...
```text
Step 1: รับค่าเลขมาก่อนสองค่า
Step 2: ทำให้เป็นจำนวนเต็ม และแยกเป็น X Y
Step 3: ใช้ total เพื่อเก็บระยะทาง และ count เพื่อนับการกระโดด
Step 4: ใช้ for loop สำหรับการกระโดด โดยเริ่มจาก X และลดระยะครั้งละ 2 เมตร
Step 5: นำค่า jump ไปบวกกับ total และเพิ่ม count ขึ้นทีละ 1
Step 6: ตรวจสอบเงื่อนไข total >=y
Step 7: ถ้าจริง ให้แสดง count
Step 8: ถ้าไม่จริง ให้แสดง -1
```
...
```text
เหมือนกับแผนแรก ฉันใช้ for loop ในการจำลองการกระโดดของกบ เริ่มจาก X และ X ลดลงครั้งละ 2 พร้อมกับสะสมระยะทางและนับการกระโดด เมื่อระยะตรงตามเงื่อนไขก็แสดงจำนวนการกระโดด หากไม่ตรงจะแสดง -1
```
...
```text
เช็คกรณีระยะทางตรงเงื่อนไข ถึงเป้าหมายพอดี
```
...
```text
เช็คกรณีเงื่อนไขเกินระยะเป้าหมาย
```
