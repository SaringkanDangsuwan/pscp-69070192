...
```text
โจทย์ให้สร้างสะพานความยาว goal โดยใช้อิฐสองแบบคือใหญ่กับเล็ก โดยต้องใช้อิฐใหญ่ให้หมดก่อนค่อยนำอิฐเล็กมาโปะ

Input:
บรรทัดแรก จำนวนอิฐเล็ก
บรรทัดสอง จำนวนอิฐใหญ่
บรรทัดสาม goal

Output:
จำนวนอิฐที่ใช้ และหากสร้างสะพานไม่ได้จะแสดง -1
```
...
```text
Step 1: รับจำนวนอิฐเล็ก จำนวนอิฐใหญ่ และความยาว goal
Step 2: คำนวณจำนวนอิฐใหญ่ที่สามารถใช้ได้มากที่สุด โดยต้องไม่เกินจำนวนอิฐใหญ่ที่มี และไม่ทำให้ความยาวเกิน goal
Step 3: นำความยาวของอิฐใหญ่ที่ใช้ไปลบออกจาก goal เพื่อหาจำนวนอิฐเล็กที่ต้องใช้
Step 4: ตรวจสอบว่าจำนวนอิฐเล็กที่ต้องใช้น้อยกว่าหรือเท่ากับจำนวนที่มีหรือไม่
Step 5: ถ้ามีอิฐเล็กเพียงพอ ให้แสดงจำนวนอิฐเล็กที่ต้องใช้
Step 6: ถ้าอิฐเล็กไม่เพียงพอ ให้แสดง -1
```
...
```text
วิธีสุดท้ายของเหมือนกับแผนแรก โดยเลือกใช้อิฐใหญ่ให้ได้มากที่สุด และใช้ min() เพื่อไม่ให้ใช้เกินจำนวนที่มีหรือเกินความยาว goal จากนั้นคำนวณจำนวนอิฐเล็กที่ต้องใช้เติมส่วนที่เหลือ ถ้าอิฐเล็กมีเพียงพอก็แสดงจำนวนที่ใช้ แต่ถ้าไม่เพียงพอจะแสดง -1
```
...
```text
ตรวจสอบกรณีทั่วไปที่มีอิฐใหญ่และอิฐเล็กเพียงพอ
```
...
```text
ตรวจสอบกรณีที่อิฐเล็กไม่เพียงพอ
```
