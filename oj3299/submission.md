...
```text
โจทย์ให้หาแถบของช่องสุดท้ายที่ปลูกดอกไม้
Input:
รับค่า L คือความหนาแถบ และ N คือจำนวนช่องที่ปลูก

Output:
หมายเลขของแถบที่ช่องสุดท้ายปลูก

Constraints:
input ค่า L ยาวได้ 1-10 ค่า N ยาวได้ 1-100000
```
...
Step 2: กำหนดตัวแปร x = 1 เพื่อแทนหมายเลขแถบที่กำลังตรวจ
Step 3: หาจำนวนแนวทแยงทั้งหมดที่ครอบถึงแถบ x โดยใช้ x * L
Step 4: ใช้สูตร n(n+1)/2 เพื่อหาจำนวนช่องสะสมตั้งแต่แนวแรกจนถึงแนวสุดท้ายของแถบ x
Step 5: ถ้าจำนวนช่องสะสมยังน้อยกว่า N ให้เพิ่ม x ขึ้น 1 แล้วตรวจแถบถัดไป
Step 6: ทำซ้ำจนจำนวนช่องสะสมมากกว่าหรือเท่ากับ N
Step 7: พิมพ์ค่า x ซึ่งเป็นหมายเลขแถบที่ช่องลำดับที่ N อยู่
```
...
```text
เหมือนกับแผนแรกโดยจะไล่ตรวจทีละแถบ ใช้ x แทนเลขแถบ แล้วคำนวณจำนวนช่องสะสมถึงปลายแถบนั้น ถ้ายังไม่ถึง N ก็เพิ่ม x แล้วตรวจแถบถัดไปเรื่อย ๆ จนเจอแถบแรกที่มีจำนวนช่องสะสมถึงหรือเกิน N จากนั้นพิมพ์ x เป็นคำตอบ
```
...
```text
ดูค่าต่ำสุดของโจทย์ ก็คือกรณี = 1 เพื่อดูว่าอยู่ในแถบแรกไหม
```
...
```text
ดูกรณ๊ที่เลขอยู่ช่องสุดท้ายของแถบพอดี เพื่อเช็คเงื่อนไข while
```
...
```text
ดูกรณีที่เลขอยู่ค่าแรกของแถบใหม่  และการทำงานโดยรวม
```
