...
| OJ submission ID, if submitted | 576541 |
| OJ status | Pass |
...
```text
ให้ AI ช่วยดูเงื่อนไขกรณีคะแนนต่ำที่สุด ว่าควรคำนวณประมาณใด
```
...
```text
AI ช่วยให้สังเกตว่าคนที่ได้คะแนนต่ำสุดต้องต่ำที่สุดเท่าที่จะเป็นไปได้ max-min เกิน 2 หรือไม่ และถ้าจะให้คะแนนต่ำที่สุด ต้องให้คะแนนของสอนคนเท่ากัน รวมถึงการใช้ max เพื่อป้องกันไม่ให้คะแนนน้อยสุดติดลบ
```
...
```text
ไปแก้ไขสูตรคำนวณหาผลต่างคะแนน และใช้ max ป้องกันค่าติดลบในส่วน lowest
```
...
```text
ฉันได้เรียนรู้ว่าโจทย์ดูคะแนนที่เป็นไปได้ ไม่ใช่แค่ค่าที่โจทย์ให้มา ฉันเข้าใจว่าการให้สองคนได้คะแนนสูงสุด จะช่วยหาคะแนนต่ำสุดที่เป็นไปได้จาก total - 2 * highest และต้องใช้ max เพราะคะแนนติดลบไม่ได้ นอกจากนี้ ฉันได้ฝึกตรวจสอบเงื่อนไขด้วย testcase หลายแบบ เพื่อให้แน่ใจว่าโค้ดแยก output ได้ถูกต้อง
